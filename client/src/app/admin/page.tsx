    ...
    <main className="min-h-screen bg-neutral-950 text-neutral-100">
      ...
      <div className="mx-auto max-w-7xl px-5 py-8">
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <CreditManager user={selected} saving={saving} onChange={setSelected} onSave={saveSubscription} />
        </section>
      </div>
    </main>
  )
...
function CreditManager({ user, saving, onChange, onSave }: { user: AdminUser | null, saving: boolean, onChange: (user: AdminUser | null) => void, onSave: (event: FormEvent) => void }) {
  if (!user) return <aside className="flex min-h-[320px] flex-col items-center justify-center rounded-xl border border-dashed border-neutral-800 bg-neutral-900/20 p-8 text-center"><KeyRound size={22} className="text-neutral-600" /><h2 className="mt-4 font-medium">Select a user</h2><p className="mt-2 text-sm leading-6 text-neutral-500">Click on any account to assign or top up credits in real time.</p></aside>
  ...
  const addCredits = (amount: number) => onChange({ ...user, total_credits: Number(user.total_credits) + amount })
  const resetUsed = () => onChange({ ...user, used_credits: 0 })

  return <aside className="rounded-xl border border-neutral-800 bg-neutral-900/30 p-5"><h2 className="font-medium text-base">Assign Account Credits</h2><p className="mt-1 truncate text-xs text-neutral-400">{user.email}</p><div className="mt-4 rounded-lg border border-neutral-800 bg-black/50 p-4"><div className="flex items-center justify-between"><span className="text-xs text-neutral-400 font-medium">Available Balance</span><span className="rounded-full bg-emerald-950/80 border border-emerald-800/60 px-2.5 py-0.5 text-xs font-semibold text-emerald-400">{remaining} credits</span></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs text-neutral-400 border-t border-neutral-800/60 pt-2.5"><div>Allocated: <span className="font-semibold text-neutral-200">{user.total_credits}</span></div><div>Used: <span className="font-semibold text-neutral-200">{user.used_credits}</span></div></div></div><div className="mt-4"><label className="block text-xs font-medium text-neutral-400 mb-2">Quick Top-up Presets</label><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => addCredits(50)} className="rounded-lg border border-neutral-700 bg-neutral-900 py-1.5 text-xs font-medium text-neutral-200 hover:bg-neutral-800 hover:border-neutral-600">+50 Credits</button><button type="button" onClick={() => addCredits(100)} className="rounded-lg border border-neutral-700 bg-neutral-900 py-1.5 text-xs font-medium text-neutral-200 hover:bg-neutral-800 hover:border-neutral-600">+100 Credits</button><button type="button" onClick={() => addCredits(500)} className="rounded-lg border border-neutral-700 bg-neutral-900 py-1.5 text-xs font-medium text-neutral-200 hover:bg-neutral-800 hover:border-neutral-600">+500 Credits</button><button type="button" onClick={resetUsed} className="rounded-lg border border-neutral-700 bg-neutral-900 py-1.5 text-xs font-medium text-amber-400 hover:bg-neutral-800 hover:border-amber-700">Reset Used (0)</button></div></div><form onSubmit={onSave} className="mt-5 space-y-4"><div className="grid grid-cols-2 gap-3"><Field label="Total Credits"><input min="0" type="number" value={user.total_credits} onChange={e => set('total_credits', Number(e.target.value))} className="admin-input" /></Field><Field label="Used Credits"><input min="0" type="number" value={user.used_credits} onChange={e => set('used_credits', Number(e.target.value))} className="admin-input" /></Field></div><button disabled={saving} className="flex w-full items-center justify-center gap-2 rounded-lg bg-white px-3 py-2.5 text-sm font-semibold text-black hover:bg-neutral-200 disabled:opacity-50">{saving && <Loader2 size={15} className="animate-spin" />}{saving ? 'Assigning Credits…' : 'Save Credit Allocation'}</button></form></aside>
}
