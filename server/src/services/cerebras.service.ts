...
export async function generateExtension(
  ...
  if (!apiKey) {
    throw new Error("Cerebras API Key is not configured. Please add your CEREBRAS_API_KEY to server/.env");
  }
  ...
  try {
    const responseStream = await cerebras.chat.completions.create({
      model: "gpt-oss-120b",
      messages,
      stream: true,
      temperature: 0.2,
